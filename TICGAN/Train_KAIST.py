...
from Helpers.utilities import normalize_batch, validation_methods, FeatureExtractor
...
def run(opt, device):
    ...
    optimizer_g = optim.Adam(net_g.parameters(), lr=opt.lr, betas=(0.5, 0.999))
    optimizer_d = optim.Adam(net_d.parameters(), lr=opt.lr, betas=(0.5, 0.999))
    ...
    for epoch in range(opt.num_epochs):
        ...
        for data, target in train_bar:
            ...
            for r, f in zip(real_b_Fexs, fake_b_Fexs):
                loss_g_percept += criterionL1(r, f)
            ...
            train_bar.set_description(desc='[%d/%d] lrg: %.e, lrd: %.e, loss_d: %.6f , loss_g: %.6f'
                                           % (epoch, opt.num_epochs, opt.lr, opt.lr,
                                              running_results['loss_d'] / running_results['batch_sizes'],
                                              running_results['loss_g'] / running_results['batch_sizes']))

        totalLoss.append([running_results['loss_g'] / running_results['batch_sizes']])
        ...
        if (epoch + 1) % 1 == 0 and epoch >= 0:
            ...
            totalpsnr.append([valing_results['psnr'] / valing_results['batch_sizes'],
                              valing_results['ssim'] / valing_results['batch_sizes']])
